...
> 博士论文字稿体例之**工作整合版**（前、中、后期工作与双轨结果）。

> **材料索引**
> - 正文：[`report_academic_full_integrated.md`](../report_academic_full_integrated.md)
> - 预后主分析单行本：[`report_academic_integrated.md`](../report_academic_integrated.md)
> - 中期/合作者抽取：`report_assets/midterm/_extracted_body.md`、`report_assets/zhao/_extracted_body.md`
> - 主脚本：[`deep_experiment_full_gpu.py`](../deep_experiment_full_gpu.py)；冠军输出：`outputs_clinical_endpoint_n320_bs400/`（附图见附录 B）

**符号**：Bootstrap **B=400**；`endpoint`、`time` → 第二章 **§2.1**；**1095 d** 常用作日历常数或历史 sprint，勿等同于 **`endpoint`** 列。
...
**目的**：多中心短轴 CMR 场景下 HCM 风险分层；按博士论文章次归纳前、中、后期及阶段二双轨，厘清任务边界。

**方法**：共用短轴 U-Net（**§2.2**）。阶段一：张量、HDF5、3D CNN/区域与传统模型。阶段二并行：（1）流形 + 2D+t；（2）几何 + **AHA 17 节段** + Hilbert 相位不同步，固定外层划分，Cox/树/MLP/融合 + 校准、DCA、Bootstrap、入组敏感性。

**结果**：中期与流形侧呈现域移、Rare-event 与运动学可读差异；冠军批次（`outputs_clinical_endpoint_n320_bs400`，**第五章**）融合外测 **AUC≈0.786**；0.5 阈 **BA≈0.701 / F1≈0.48**；OOF 主阈（≈0.259）**Sens≈0.64 / Spec≈0.71 / NPV≈0.91**。**full-cohort** AUC≈**0.58**。跨章指标按**第六章**规则并读。

**结论**：工程可学性、流形运动学与终点对齐预后互为补充；预后数字以**第五章**为准，第三、四章各从其监督定义。
...
**Objective.** HCM risk stratification from multi-center short-axis CMR; dissertation-style synthesis of early and parallel late-stage arms with explicit task boundaries.

**Methods.** Shared U-Net segmentation (§2.2). Stage one: tensor pipeline and voxel/regional deep and classical models. Stage two in parallel: Kendall shape space with 2D+t versus geometric + 17-segment phase features with Excel `endpoint`/`time`, fixed stratified split, Cox and tree/MLP/ensemble, calibration, DCA, bootstrap, cohort sensitivity.

**Results.** Early and manifold arms show domain shift and rare-event constraints plus interpretable motion patterns. Champion batch: ensemble test AUC **≈0.786**, BA **≈0.701**, F1 **≈0.48** at 0.5; OOF primary threshold: Sens **≈0.64**, Spec **≈0.71**, NPV **≈0.91**. Full-cohort AUC **≈0.58**. Cross-arm comparisons require explicit task definitions (Ch. 6).

**Conclusion.** Engineering diagnostics, manifold kinematics, and endpoint modeling are complementary; prognosis figures refer to Chapter 5; Chapters 3–4 stand on their own supervision. Clinical synthesis in Chapter 7.
...
肥厚型心肌病（HCM）常见且表型异质大，不良事件风险差异显著，风险分层直接影响干预决策。心脏磁共振（CMR）可无创定量刻画心肌形态与功能，广泛用于多中心队列。多中心数据在帧数、层厚、场强与体素间距上存在系统差异，影像—掩膜—随访构成高维异质链路；「能否学习、如何表征、能否对齐终点」须在**可追溯流程**下分阶段、多路径回答，难以单模型一次解决。本文按博士论文章次归纳课题组前、中、后期工作。
...
> **编排**：“一、引言”已并入第一章；本节自数据处理与实验摘起。全文见 [`report_assets/midterm/_extracted_body.md`](../report_assets\midterm\_extracted_body.md)。
...
| 准确率         0.8862（总样本数： 791） 宏平均         0.5726         0.5636           0.5676           791 加权平均       0.8789         0.8862           0.8825           791 |  |  |  |  |
...
| Class 0 | 690（真阴性） 41（假阳性） |

| Class 1 | 49（假阴性）  11（真阳性） |
...
| 准确率           0.7016（总样本数： 791） 宏平均   0.5767      0.7468       0.5502              791 加权平均  0.9162      0.7016       0.7716             791 |  |  |  |  |
...
| Class 0 | 507（真阴性） 224（假阳性） |

| Class 1 | 12（假阴性）  48（真阳性） |
...
| 正常 患病 | 0.96 0.17 | 0.92 0.33 | 0.94 0.22 | 59 3 |

| 准确率 宏平均  加权平均 | 0.57 0.93 | 0.89 0.62 0.89 | （总样本数：62） 0.58 0.90 | 62 62 |
...
| 正常 患病 | 0.95 0.00 | 0.97 0.00 | 0.96 0.00 | 59 3 |

| 准确率 宏平均  加权平均 | 0.47 0.90 | 0.92 0.48 0.92 | （总样本数：62） 0.48 0.91 | 62 62 |
...
| 正常（低风险）  538（真阴性） 患病（高风险）  109（假阴性） | 5（假阳性） 4（真阳性） |
...
| 数据来源 (中心/队列) | 原始数据目录HCM 生存组 (0) | HCM 死亡组 合计 (1) |  |  |
...
| ID | 输出目录（相对仓库根目录 `） | 入组 / 要点 | 树/MLP/融合标签 | 备注 |
|----|--------------------------------------|-------------|-----------------|------|
...
```powershell
python "export_md_to_docx.py" "02_report_academic_full_integrated/report_academic_full_integrated.md" --base-dir "." -o "report_academic_full_integrated.docx"
```
...
*整合说明：临床主分析数值与附图以当前 `outputs_*` 批次为准。*
